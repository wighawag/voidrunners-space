...
export class MultiplayerSystem implements System<SupportedComponents> {
  ...
  addAvatar(
    ...
    if (this.entities[tokenID]) {
      ...
      const voidrunnerID = tokenID;
      // const voidrunnerID = Math.floor(Math.random() * 48) + 3;
      const model: CModel['model'] = {
        scale: 0.1,
        rotation: { rx: 0, ry: Math.PI, rz: 0 },
        source: `${serverURL}/voidrunners/${voidrunnerID}.glb`,
        type: 'gltf',
      };
