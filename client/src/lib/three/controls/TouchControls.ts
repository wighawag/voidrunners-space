...
type Config = {
  ...
  hitTest: boolean; // stop on hitting objects
  hitTestDistance: number; // distance to test for hit
};

export class TouchControls {
  protected rotationPad: RotationPad;
  protected movementPad: MovementPad;
  protected container: HTMLElement;
  protected config: Config;
  protected fpsBody: Object3D;
  protected mouse: Vector2;
  protected enabled = true;
  protected scene: Scene;
  protected rotationMatrices: Matrix4[];
  protected hitObjects;
  protected velocity: Vector3;
  protected cameraHolder: Object3D;
  protected maxPitch: number;
  protected isRightMouseDown: boolean = false;
  protected moveForward: boolean = false;
  protected moveBackward: boolean = false;
  protected moveLeft: boolean = false;
  protected moveRight: boolean = false;
  protected moveForwardLocked: boolean = false;
  protected moveBackwardLocked: boolean = false;
  protected moveLeftLocked: boolean = false;
  protected moveRightLocked: boolean = false;
  protected ztouch: number = 1;
  protected xtouch: number = 1;
  ...
  constructor(
    ...
    this.fpsBody = new Object3D();
    this.fpsBody.add(this.cameraHolder);

    // Creating rotation pad
    this.rotationPad = new RotationPad(container, canvas);
    this.rotationPad.padElement.addEventListener(
      'YawPitch',
      (event: CustomEvent<{ deltaX: number; deltaY: number }>) => {
        let rotation = this.calculateCameraRotation(
          event.detail.deltaX,
          event.detail.deltaY,
        );
    ...
    this.movementPad = new MovementPad(container, canvas);
    this.movementPad.padElement.addEventListener(
      'move',
      (
        event: CustomEvent<{ deltaX: number; deltaY: number; middle: number }>,
      ) => {
  ...
  // Private functions
  //
  protected prepareRotationMatrices() {
    let rotationMatrixF = new Matrix4();
    rotationMatrixF.makeRotationY(0);
    this.rotationMatrices.push(rotationMatrixF); // forward direction

    let rotationMatrixB = new Matrix4();
  ...
  protected calculateCameraRotation(dx: number, dy: number, factor?: number) {
  ...
  protected lockDirectionByIndex(index) {
    if (index == 0) this.lockMoveForward(true);
    else if (index == 1) this.lockMoveBackward(true);
    else if (index == 2) this.lockMoveLeft(true);
    else if (index == 3) this.lockMoveRight(true);
  }
  ...
  //
  update() {
    if (this.config.hitTest) this.hitTest();
    ...
    if (this.moveForward && !this.moveForwardLocked) {
      this.velocity.z -=
        this.ztouch * this.config.moveSpeed * this.config.delta;
    }
    ...
    if (this.moveLeft && !this.moveLeftLocked) {
      this.velocity.x -=
        this.xtouch * this.config.moveSpeed * this.config.delta;
    }
  ...
  hitTest() {
    ...
    this.hitObjects = [];
    let cameraDirection = this.getDirection2(new Vector3(0, 0, 0)).clone();
    ...
    for (let i = 0; i < 4; i++) {
      ...
      let rayCaster = new Raycaster(this.fpsBody.position, direction);
      let intersects = rayCaster.intersectObject(this.scene, true);
      if (
        intersects.length > 0 &&
        intersects[0].distance < this.config.hitTestDistance
      ) {
  ...
  setRotation(x, y) {
    // let camHolder = this.fpsBody.getObjectByName('cameraHolder')
    if (x !== null) this.cameraHolder.rotation.x = x;
